...
├─ notebook/              # 분석 노트북(main, KPI, funnel, behavior)
├─ overview/              # 개요, 데이터사전, 분석가이드, ERD, CEO 질문
├─ image/                 # 보조 시각화(Aha, Bounce, CVR, funnel, Retention)
├─ outputs/
│  ├─ DA_results/         # 최종 핵심 시각화
│  └─ ppt/                # 발표 PDF 및 보고서형 흐름 문서
├─ pda/
│  ├─ AARRR/              # AARRR 프레임워크 문서
│  └─ A_B_Test/           # A/B 테스트 설계 문서
├─ table/                 # 전처리/집계 테이블
└─ README.md
```
